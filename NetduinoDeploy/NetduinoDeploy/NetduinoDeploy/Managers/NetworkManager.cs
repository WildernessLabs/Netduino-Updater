...
namespace NetduinoDeploy.Managers
{
	public class NetworkManager
	{
		...
		HAL_CONFIGURATION_SECTOR staticConfig;
		byte[] all_cfg_data = null;
		int lastCfgIndex = -1;
		Hashtable cfgHash = new Hashtable();
		const UInt32 version_V2 = 0x324C4148; // HAL2
		const UInt32 seed = 1;          // HAL_STRUCT_VERSION
		const UInt32 enumerateAndLaunchAddr = 0x0;
		const int maxDriverNameLength = 63;
		...
		private void WriteConfig(string configName, byte[] data, bool staticSize, bool updateConfigSector)
		{
			...
			else        // adding a new configuration to the end of the current list
			{
				...
				if (lastCfgIndex == -1) throw new OutOfMemoryException();
